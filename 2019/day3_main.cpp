...
void part_one(const std::vector<std::string>& lines)
{
    ...
    std::cout << "[Part One] The minimum distance for an intersection is: " << min_distance << '\n';
}
...
void part_two(const std::vector<std::string>& lines)
{
    ...
    for (const coord& c : line1_visited)
    {
        ...
        {
            coord new_coord = c;
            new_coord.step_count += line2_visited.find(c)->step_count;
            intersection.insert(new_coord);
        }
    }
    ...
    for (const coord& c : intersection)
    {
        if (c.step_count < min_distance)
        {
            min_distance = c.step_count;
        }
    }

    std::cout << "[Part Two] The minimum distance for an intersection is: " << min_distance << '\n';
}
