...
struct distance_location
{
    std::string from;
    std::string to;
    int distance;

    distance_location(const std::string& from_, const std::string& to_, int distance_)
        : from(from_)
        , to(to_)
        , distance(distance_) { }
};
...
int main()
{
    std::fstream file{ jumi::open_file("Day9_input.txt") };
    std::vector<std::string> file_contents{ jumi::read_lines(file) };
